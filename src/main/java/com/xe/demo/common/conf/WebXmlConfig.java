...
public class WebXmlConfig extends WebMvcConfigurerAdapter {
	...
	public void addInterceptors(InterceptorRegistry registry) {
		//LoginRequestInterceptor自定义拦截器类
	...
	public ServletListenerRegistrationBean<EventListener> getDemoListener() {
		...
		registrationBean.setListener(new AuthorityListener());//设置权限监听器AuthorityListener为监听自类
	 ...
	 * @return
	 */
	@Bean//注册访问druid的application.properties参数
	public ServletRegistrationBean druidServletRegistration() {
		ServletRegistrationBean registration = new ServletRegistrationBean(new StatViewServlet(), "/druid/*");
		// 添加初始化参数：initParams

		// IP白名单：
		registration.addInitParameter("allow", env.getProperty("druid.allow"));//127.0.0.1
