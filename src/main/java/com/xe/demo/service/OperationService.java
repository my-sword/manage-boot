...
public class OperationService extends AbstratService<AuthOperation> {
	...
	public void initAuthority() {
		//MapperKey：根据包位置+字段id 查找Mapper集合
		...
		if (CollectionUtils.isNotEmpty(operationList)) {
			for (AuthOperation operation : operationList) {
				opIdList.add(operation.getOpid() + "");//将所有权限添加到opIdList(添加主键id 并转为String)
			}
		}

		// 要保存的列表
		List<AuthOperation> saveList = new ArrayList<>();
		// 要修改的列表
		List<AuthOperation> updateList = new ArrayList<>();

		// 同步json文件(左侧菜单栏权限--权限整合日志) 将json转化为集合
		...
		if (operationJsonList != null) {
			for (AuthOperation operation : operationJsonList) {
				//查找对应的单权限对象  findForObject（,operation查找字段内的具体值对应的整个对象）
		...
		// 同步注解里面的权限  获取com.xe.demo.controller包里所有的controller类方法
		...
		for (Map.Entry<String, Authority> entry : entrySet) {
			...
			System.out.println(permission+" href");
		...
		if (null == role) {
			...
			role.setCname("匿名用户组");
			dao.save(MapperKey.ROLE_addAdmin, role);//添加管理员角色
		}
